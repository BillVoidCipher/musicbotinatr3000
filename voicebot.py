...
import config   # pip install config

# Import Speech Recognition Library  (pip install SpeechRecognition)
import speech_recognition as sr

# Import Windows 10 TTS Library   (python -m pip install pywin32)
        ...
        try:
            if ctx.message.author.server_permissions.administrator:
                ...
                with sr.Microphone() as source:
                    # Распечатать и произнести стартовое сообщение
                    print(startup_mesg)
                    speak.Speak(startup_mesg)
                    # Слушайте микрофон
                    audio = r.listen(source)

                # Сохраняем голосовой ввод в переменной
